...
namespace Traceless.SocketIO
{
    public class Client : IDisposable, IClient
    {
        ...
        // Events
        /// <summary>
        /// Opened event comes from the underlying websocket client connection being opened. This is
        /// not the same as socket.io returning the 'connect' event
        /// </summary>
        ...
        /// <para>
        /// The Socket.IO service may have closed the connection due to a heartbeat timeout, or the
        ...
        /// <summary>
        /// Represents the initial handshake parameters received from the socket.io service (SID,
        /// HeartbeatTimeout etc)
        /// </summary>
            ...
            lock (padLock)
            {
                if (!(this.ReadyState == WebSocketState.Connecting || this.ReadyState == WebSocketState.Open))
                {
                    try
                    {
                        ...
                        // this.HandShake = this.requestHandshake(uri);// perform an initial HTTP
                        // request as a new, non-handshaken connection
        ...
        /// <para>
        /// Reserved socket.io event names available: connect, disconnect, open, close, error,
        /// retry, reconnect
        /// </para>
        /// </summary>
        /// <param name="eventName"></param>
        /// <param name="action"></param>
        /// <example>client.On("testme", (data) =&gt; { Debug.WriteLine(data.ToJson()); });</example>
        ...
        /// <para>
        /// Do not use the reserved socket.io event names: connect, disconnect, open, close, error,
        /// retry, reconnect
        /// </para>
        /// </summary>
        /// <param name="eventName"></param>
        ...
        public void Emit(string eventName, dynamic payload, string endPoint = "", Action<dynamic> callback = null)
        {
            ...
            switch (lceventName)
            {
                ...
                case "retry":
                case "reconnect":
                    Trace.WriteLine(string.Format("Emit ArgumentOutOfRangeException: {0}", msg.RawMessage));
        ...
        /// <summary>
        /// Raw websocket messages from server - convert to message types and call subscribers of
            ...
            if (this.ReadyState == WebSocketState.Open)
            {
                ...
                try
                {
                    if (this.outboundQueue != null && !this.outboundQueue.IsAddingCompleted)
                    {
                        ...
                        {
                            Task.Run(() => this.HeartBeatTimerEvent.Invoke(this, EventArgs.Empty));
                        }
                    }
                }
                catch (Exception ex)
        ...
        /// <summary>
        /// <para>
        /// Client performs an initial HTTP POST to obtain a SessionId (sid) assigned to a client,
        /// followed by the heartbeat timeout, connection closing timeout, and the list of supported transports.
        /// </para>
